...
function getWeather() {
	return fetch(API_URL).then((res) => res.json()).then((data) => {
		/* deconstruye la data a solo la key, vl checks y los dias  */
		const { sol_keys, validity_checks, ...solData } = data;
		/* las entries recorre la data deconstruida y mapea nuevos arrays para cada dia con los datos pedidos */
		return Object.entries(solData).map(([ sol, data ]) => {
			/* el map data solo toma la key del sol y la demas data para filtrarla en el return  */
			return {
				sol: sol,
				maxTemp: data.AT.mx,
				minTemp: data.AT.mn,
...
/* retorna la promise y los arrays de los sols */
getWeather().then((sols) => {
	/* selecciona el current day */
...
/* mostrar los datos del sol actual  */
...
/* manage units updates and see which one is checked */
